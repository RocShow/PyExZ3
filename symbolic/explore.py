...
class ExplorationEngine:
    def __init__(self, funcinv, solver="z3"):
        ...
        # link up SymbolicObject to PathToConstraint in order to intercept
        # control-flow
    ...
    def _updateSymbolicParameter(self, name, val):
        self.symbolic_inputs[name] = self.invocation.createArgumentValue(name,
                                                                         val)
    ...
    def _isExplorationComplete(self):
        ...
        if num_constr == 0:
            ...
            log.info(
                "%d constraints yet to solve (total: %d, already solved: %d)" % (
                num_constr, self.num_processed_constraints + num_constr,
                self.num_processed_constraints))
            return False
