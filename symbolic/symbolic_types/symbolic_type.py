...
class SymbolicType(object):
    ...
    def _do_sexpr(self, args, fun, op, wrap):
        unwrapped = [(a.unwrap() if isinstance(a, SymbolicType) else (a, a)) for
                     a in args]
    ...
    def _eq_worker(self, expr1, expr2):
        ...
        if isinstance(expr1, list):
            return len(expr1) == len(expr2) and \
                   ...
                   all([self._eq_worker(x, y) for x, y in
                        zip(expr1[1:], expr2[1:])])
    ...
    def _toString(self, expr):
        if isinstance(expr, list):
            return "(" + expr[0] + " " + ", ".join(
                [self._toString(a) for a in expr[1:]]) + ")"
...
class SymbolicObject(SymbolicType, object):
    ...
    def __eq__(self, other):
        ...
        return self._do_bin_op(other, lambda x, y: x == y, "==",
                               SymbolicObject.wrap)
