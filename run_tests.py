...
parser = OptionParser()
parser.add_option("--cvc", dest="cvc", action="store_true",
                  help="Use the CVC SMT solver instead of Z3", default=False)
parser.add_option("--z3", dest="cvc", action="store_false",
...
for f in files:
    ...
    with open(os.devnull, 'w') as devnull:
        ...
        ret = subprocess.call(
            [sys.executable, "pyexz3.py", "--m=25", solver, full],
            stdout=devnull)
    if (ret == 0):
