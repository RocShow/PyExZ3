...
# q1 -[0..9]-> q2
def q1axiom():
    body = q1(y) == And(y != nil, ULE(ord('0'), head(y)),
                        ULE(head(y), ord('9')), \
                        q2(tail(y)))
...
def q2axiom():
    body = q2(y) == Or(y == nil, And(y != nil, ULE(ord('0'), head(y)),
                                     ULE(head(y), ord('9')), q2(tail(y))))
    return ForAll(y, body, patterns=[q2(y)])


# q3 -[a-z]-> q4
def q3axiom():
    body = q3(y) == And(y != nil, ULE(ord('a'), head(y)),
                        ULE(head(y), ord('z')), q4(tail(y)))
